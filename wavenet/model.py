...
class WaveNetModel(object):
    ...
    def _create_dilation_layer(self, input_batch, layer_index, dilation, output_width,
        ...
        if global_condition is not None:
            ...
            filter_prod = tf.matmul(global_condition, weights_gcond_filter)
            gate_prod =  tf.matmul(global_condition, weights_gcond_gate)
           
            conv_filter = conv_filter + filter_prod
            conv_gate = conv_gate + gate_prod
    ...
    def predict_proba(self, samples, global_condition=None,
        ...
        samples_shape = samples.get_shape().as_list()
    ...
    def loss(self,
        ...
        with tf.name_scope(name):
            ...
            if local_condition is not None:
                ...
                lc_encoded = tf.reshape(lc_encoded, [-1, self.local_channels])
            else:
                lc_encoded = None
